    ...
    try {
        ...
        if (userDoc) {
            ...
            if (passOK) {
                ...
                res.cookie('token', token, { httpOnly: false, domain: '.onrender.com',secure: true, sameSite: "none", domain: "localhost", expires: new Date(Date.now() + 5 * 60 * 60 * 1000) }).json({ status: "ok", data: token, userDoc });
            } else {
